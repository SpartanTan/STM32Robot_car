...
//	USART1_SendByte(0x09);
//	USART1_SendByte(channel);
//	
//	ch_Temp ^= 0x09;
//	ch_Temp ^= channel;
//	ch_Temp ^= Temp_B_H;
...
//	USART1_SendByte(Temp_A_L);
//	USART1_SendByte(Temp_A_H);
//	USART1_SendByte(Temp_B_L);
//	USART1_SendByte(Temp_B_H);

//	USART1_SendByte(ch_Temp);
...
void SendHex(uint8_t hex) {
  ...
  temp = hex >> 4;
  if(temp < 10) {
    USART1_SendByte(temp + '0');
  } else {
    USART1_SendByte(temp - 10 + 'A');
  }
  temp = hex & 0x0F;
  if(temp < 10) {
    USART1_SendByte(temp + '0');
